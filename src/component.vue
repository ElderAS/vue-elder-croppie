  ...
  <div class="elder__croppie" :style="background">
    <div class="elder__croppie-actions" v-if="value || instance">
      <template v-if="!instance">
        <button type="button" class="elder__croppie-button elder__croppie-button--error" @click="reset">
          <fa icon="times"></fa>
        </button>
        <button type="button" class="elder__croppie-button" @click="$refs.file.click()">
      ...
      <template v-else>
        <template v-if="canRotate">
          <button type="button" class="elder__croppie-button" @click="rotate(90)">
            <fa icon="undo"></fa>
          </button>
          <button type="button" class="elder__croppie-button" @click="rotate(-90)">
    ...
    <div v-if="showPlaceholder" class="elder__croppie-placeholder">
      <button-component type="button" primary :label="placeholder" icon="plus" @click="$refs.file.click()"/>
    </div>
    <div ref="croppie"></div>
    <input ref="file" type="file" accept="image/*" class="elder__croppie-upload" @change="prepare($event)" />
  </div>
</template>
...
export default {
  ...
  methods: {
    parseValue() {
      if (!this.value || typeof this.value === 'string') return (this.parsedValue = this.value)
...
$vue-elder-croppie--background: #eaeaea !default;
...
.elder__croppie {
  ...
  background-color: $vue-elder-croppie--background;
